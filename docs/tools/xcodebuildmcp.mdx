...
This playbook prefers **MCP names** (coding agents’ primary path). When the host has shell but no MCP tools, use the CLI column.
...
5. Report active context (project/workspace, scheme, simulator/device) and the **exact failing tool** on errors.
...
If a tool is missing from the host list, enable the workflow and restart MCP — do not fall back to raw `xcodebuild` without reporting the blocker. Upstream live catalog: [Tools reference](https://xcodebuildmcp.com/docs/tools).
:::
...
## CLI recipes (shell / CI / agents without MCP tools)

Same config file as MCP. Synopsis: `xcodebuildmcp <workflow> <tool> [options]`.
...
| Agent used raw `xcodebuild` | Prefer MCP; if MCP unavailable, use CLI recipes above or report blocker |
...
Constraints: XcodeBuildMCP MCP tools if available; else CLI. No silent raw xcodebuild. Report residual risk.
