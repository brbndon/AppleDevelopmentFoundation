...
| **Verified against** | XcodeBuildMCP **2.7.0** tool catalog (`npx xcodebuildmcp tools`) + official docs |
...
"Report blocked" is a fail-closed contract: when no authorized capability remains,
refusal is mandatory and must leave an auditable blocked report. Exceptions require
an explicit user authorization naming the specific action and scope; tool
availability never authorizes fallback. See [Refusability contract](/reference/refusability-contract).
...
| Debug attach / breakpoints | `debugging` | Stateful (daemon in CLI mode); iOS/tvOS/watchOS/visionOS only — not macOS |
...
The table above lists the workflows referenced by this repository and is not
exhaustive; newer upstream workflows (for example `coverage`, `utilities`,
`workflow-discovery`) can be enabled on demand.
...
Exact advertised set depends on `enabledWorkflows` and XcodeBuildMCP version. Prefer the host’s live tool list. Mapping verified against **2.7.0** + official docs:
...
| Screenshot / UI snapshot | `screenshot`, `snapshot_ui` | `ui-automation screenshot`, `snapshot-ui` |
