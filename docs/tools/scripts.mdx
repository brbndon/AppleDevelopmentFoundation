...
./Scripts/install-skills.sh --status      # read-only installed-vs-HEAD reconciliation
...
| Reconcile installed skills vs HEAD (read-only) | `./Scripts/install-skills.sh --status` — exits nonzero on any dangling, wrong-target, content-drift, missing, or stale link; see [Skill sync](/reference/skill-sync) |
...
| `ok: … (matches manifest and HEAD)` | During `--status`, link is correct and content matches HEAD |
| `missing` / `dangling` / `wrong-target` / `drift` / `stale: …` | During `--status`, a divergence was found; `--status` exits nonzero |

`--status` and `--uninstall` are mutually exclusive (combining them exits 2). `--status` is read-only, so `--dry-run` is a no-op with it.
