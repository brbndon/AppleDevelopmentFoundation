...
For a consumer without UI E2E—whether it is macOS-only, visionOS/watchOS, a
...
relying on journeys. Keep secrets out of this file, flow files, fixtures, and
artifacts.

- UI E2E tool: `<E2E_TOOL_NAME_OR_NOT_CONFIGURED_OR_N_A>`
- App identifier (bundle ID): `<BUNDLE_IDENTIFIER_OR_N_A>`
...
- Exact destination/configuration: destination ID
  `<EXACT_DEVICE_OR_SIMULATOR_ID_OR_N_A>`, scheme `<SCHEME_OR_N_A>`, and build
  configuration `<CONFIGURATION_OR_N_A>`; when configured, use the same
  destination and configuration for XcodeBuildMCP and the configured UI E2E
  runner
...
- Deep links: `<SUPPORTED_DEEP_LINKS_AND_INVOCATION_OR_N_A>`
- E2E artifacts: use the UI inspection contract location and preserve failure
