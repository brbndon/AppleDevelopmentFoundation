...
targets, Swift packages, or repos without UI E2E, set:

- Maestro / UI E2E: `not configured` (or `n/a`)

and leave the remaining fields as `n/a`. Do not invent a simulator ID, `.maestro/`
layout, or onboarding flow when E2E is not in scope. Agents must not treat an
unconfigured contract as an iOS Maestro destination.

When Maestro/iOS E2E **is** in use, document the deterministic app contract
before relying on journeys. Keep secrets out of this file, flow files, fixtures,
and artifacts.

- Maestro / UI E2E: `<CONFIGURED_OR_NOT_CONFIGURED_OR_N_A>`
- App identifier (bundle ID): `<BUNDLE_IDENTIFIER_OR_N_A>`
- `.maestro/` location and naming: `<REPOSITORY_ROOT_OR_AGREED_PATH_OR_N_A>`;
  name flows `<JOURNEY>.yaml` and reusable subflows `<SUBFLOW>.yaml` using
  `<NAMING_OR_TAGGING_CONVENTION_OR_N_A>`
- Exact simulator/configuration: simulator ID
  `<EXACT_SIMULATOR_ID_OR_N_A>`, scheme `<SCHEME_OR_N_A>`, and build
...
- Launch arguments/environment: `<LAUNCH_ARGUMENTS_OR_N_A>` and
  `<ENVIRONMENT_VARIABLES_OR_CONFIG_SOURCE_OR_N_A>`; inject secrets locally or
