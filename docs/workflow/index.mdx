...
A reliable agent session follows a fixed loop. Skip steps only when the task is clearly narrower than the loop. An **unattended** run follows the stricter [Autonomous session](/workflow/autonomous-session) contract, including the installed-vs-HEAD reconciliation check and a leave-behind receipt.
...
4. **Implement in the consumer workspace**
   - Prefer Swift 6.2 or later, SwiftUI, native observation, structured concurrency, focused views.
...
## Multi-service / LAN API mesh apps

For apps that talk to several independent HTTP APIs (Keychain secrets, local/remote endpoints, phased read-only clients, `FeatureState`, inspection routes), follow the reusable patterns in [Multi-service iOS app patterns](/workflow/multi-service-ios-app). Keep product-specific severity lists and paste prompts in the consumer app’s `Documentation/`.
...
- [Task recipes](/workflow/task-recipes) — scenario → skills → verify (start here for a concrete task)
