...
Proven pattern (Harborlight). Continuous 3×3 ring + liquid-glass **orb** (no brand glyph). Harmonics, not keyframe holds. Soft glow must **not** clip to a hard square.

| Step | Action |
| --- | --- |
| Template | `Templates/LiquidOrbLoader/` — `README.md` (wiring + anti-box checklist) + drop-in `DotMatrixLoader.swift` |
| Full-screen | `FeatureLoadingView` for `FeatureState.loading`; put loader **outside** `ScrollView` so it fills the viewport |
...
| Glow | Oversized canvas (`layoutScale` ≈ 2.6); bake squash into path; no `scaleEffect` + tight frame; radial mask + transparent gradient stops |
