...
| Tools | XcodeBuildMCP |
| Verify | `session_show_defaults` → **`build_run_sim`** (prefer) → `test_sim` if present → optional `screenshot` / `snapshot_ui` |
...
| Do not | Use tab skill for page-style `TabView` content or segmented controls; put branding-specific tokens into foundation skills; land components in this repo’s archived `Sources/` |
