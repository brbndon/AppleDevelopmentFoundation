...
| Defaults wrong (project/scheme/simulator) | Stale or empty session defaults | Call `session_show_defaults` first; set defaults or use `discover_projs` **only** when defaults are missing/wrong — never parallel with show-defaults |
| `discover_projs` finds nothing useful | Wrong workspace root / multi-root monorepo | Open the consumer app root; set explicit project/workspace + scheme in session defaults or config `sessionDefaults` |
| UI automation tools missing | Workflow not enabled | Add `ui-automation` (and related) to `enabledWorkflows`; restart MCP |
| Agent falls back to raw `xcodebuild` / `simctl` | MCP unavailable or misconfigured | Report the blocker; fix MCP config before inventing CLI workflows when the host should use MCP |
