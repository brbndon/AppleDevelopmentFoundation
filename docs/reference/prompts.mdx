...
Each prompt is an **agent contract**: goal, constraints, inputs, done definition, verification. Replace `/path/to/…` and task specifics. Prefer `$skill-name` invocation your host understands.
...
- Prefer XcodeBuildMCP over raw xcodebuild/simctl.
- Do not expand AppleDevelopmentFoundation archived Sources/, Website/, or package modules.
...
- Do not land code in AppleDevelopmentFoundation archived Sources/.
...
Constraints: XcodeBuildMCP only if available; report if MCP is missing instead of silent CLI fallback.
...
Never claim pass without running the tool. MCP names above; CLI: xcodebuildmcp simulator build-and-run, etc.
```
...
- Follow Documentation/SkillAuthoringGuide.md.
- Keep skills neutral (no branding, secrets, user-specific paths, hidden network).
- Update manifest.json, .agents/skills/README.md, and master-skill.json when the router should know.
Inputs: Skill change request.
Done when: Contracts pass and docs inventory note updated if tables changed.
