...
Verification: Strict concurrency build when relevant; then [verification matrix](/reference/verification-matrix) for the change type.
...
- After each flow or journey change, run only the single smallest relevant flow — not tagged or full-suite runs unless the user or a documented release/CI gate requires them.
Inputs:
- Consumer path: <consumer path>
- App identifier: <…>
...
Verification: Targeted maestro test on the edited/affected flow; report <simulatorId> and artifacts.
...
Handoff: classifications, diffs vs expected inventory, verification outcomes if run, residual risk.
```
