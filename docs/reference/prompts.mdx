...
Verification: session_show_defaults → MCP build (prefer build-and-run) → tests if present → optional screenshot/hierarchy.
```
...
2. discover_projs only if defaults wrong/missing
3. Build (prefer build-and-run for launch)
4. Run tests if present
Never claim pass without running the tool.
```
