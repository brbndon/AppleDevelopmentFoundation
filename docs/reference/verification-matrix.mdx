...
| Reconciliation mode (`--status`) change | `./Scripts/verify-skills.sh` + `./Scripts/test-install-skills.sh` |
| Installed-vs-HEAD check before routing | `./Scripts/install-skills.sh --status`; re-sync with `--uninstall` then install on divergence — see [Skill sync](/reference/skill-sync) |
