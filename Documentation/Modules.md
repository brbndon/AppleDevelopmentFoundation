...
| MediaKit | validated image data loading/cache | silent transforms, compression, metadata stripping, or media editing | `ImageLoadPolicy`, `ImageLoader`, `ImageDataCache` | iOS/macOS; cache has no eviction policy; test cancellation/policy; add transformations only with explicit options. |
